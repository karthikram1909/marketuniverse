...
import { AlertCircle, Check, X, Plus, Trash2, Shield, Ban, Crown, Lock, Unlock, Upload, Loader2, Edit2, Settings as SettingsIcon, Search } from 'lucide-react';
...
export default function ChatRoomAdmin() {
    ...
    const { data: chatProfiles = [] } = useQuery({
        ...
        queryFn: () => base44.entities.ChatProfile.filter({})
    });
    ...
    const handleSyncUsers = async () => {
        setSyncing(true);
        try {
            await base44.functions.invoke('syncUsersToChat');
    ...
    // Use ChatProfile as single source of truth (no ghost users)
    ...
    const { data: youtubeSettings = [] } = useQuery({
        ...
        queryFn: () => base44.entities.YouTubeSettings.filter({})
    });
    ...
    // Approve user mutation
     const approveMutation = useMutation({
         mutationFn: async (profile) => {
             if (!profile.has_chat_profile) {
                 return await base44.entities.ChatProfile.create({
                     user_id: profile.id,
             ...
             return await base44.entities.ChatProfile.update(profile.chat_profile_id, { is_approved: true });
         },
         onSuccess: (data, variables) => {
             const chatProfileId = variables.chat_profile_id || data.id;
             const currentProfiles = queryClient.getQueryData(['allChatProfiles']) || [];
             const updated = currentProfiles.map(p => 
                 p.id === chatProfileId
                     ? { ...p, is_approved: true }
                     : p
             );
             queryClient.setQueryData(['allChatProfiles'], updated);
         },
         onSettled: () => {
    ...
    const blockMutation = useMutation({
        mutationFn: async (profile) => {
            ...
            return await base44.entities.ChatProfile.update(profile.chat_profile_id, { is_blocked: true });
        },
        onSuccess: (data, variables) => {
            const chatProfileId = variables.chat_profile_id || data.id;
            const currentProfiles = queryClient.getQueryData(['allChatProfiles']) || [];
    ...
    const unblockMutation = useMutation({
        mutationFn: async (profile) => {
            if (!profile.has_chat_profile) return;
            return await base44.entities.ChatProfile.update(profile.chat_profile_id, { is_blocked: false });
        },
        onSuccess: (data, variables) => {
            const chatProfileId = variables.chat_profile_id || data?.id;
            const currentProfiles = queryClient.getQueryData(['allChatProfiles']) || [];
    ...
    // Delete user mutation
     const deleteUserMutation = useMutation({
         mutationFn: async (profileId) => {
             if (!profileId) throw new Error('ChatProfile ID not found');
             return await base44.entities.ChatProfile.delete(profileId);
         },
         onSuccess: () => {
             queryClient.invalidateQueries({ queryKey: ['allChatProfiles'] });
     ...
     const setAsAdminMutation = useMutation({
         mutationFn: async (profile) => {
             ...
             return await base44.entities.ChatProfile.update(profile.chat_profile_id, { is_admin: true });
         },
         onSuccess: (data, variables) => {
             const chatProfileId = variables.chat_profile_id || data.id;
             const currentProfiles = queryClient.getQueryData(['allChatProfiles']) || [];
     ...
     // Set as user mutation
     const setAsUserMutation = useMutation({
         mutationFn: async (profile) => {
             if (!profile.has_chat_profile) return;
             return await base44.entities.ChatProfile.update(profile.chat_profile_id, { is_admin: false });
         },
         onSuccess: (data, variables) => {
             const chatProfileId = variables.chat_profile_id || data?.id;
             const currentProfiles = queryClient.getQueryData(['allChatProfiles']) || [];
    ...
    const createRoomMutation = useMutation({
        mutationFn: (roomData) =>
            base44.entities.ChatRoom.create({
                ...roomData,
                created_by: 'admin',
                is_active: true
            }),
        onSuccess: () => {
            setNewRoom({ name: '', description: '', category: '', order: 0, permission: 'read_write' });
    ...
    const updateRoomMutation = useMutation({
        mutationFn: ({ roomId, updates }) =>
            base44.entities.ChatRoom.update(roomId, updates),
        onSuccess: () => {
    ...
    const deleteRoomMutation = useMutation({
        mutationFn: (roomId) =>
            base44.entities.ChatRoom.delete(roomId),
        onSuccess: () => queryClient.invalidateQueries({ queryKey: ['allChatRooms'] })
    });

    // Update YouTube settings mutation (room-specific - deprecated)
    const updateYoutubeMutation = useMutation({
        mutationFn: async ({ roomId, youtubeUrl }) => {
            ...
            if (existing) {
                return await base44.entities.YouTubeSettings.update(existing.id, {
                    youtube_video_url: youtubeUrl
                });
            } else {
                return await base44.entities.YouTubeSettings.create({
                    chat_room_id: roomId,
                    youtube_video_url: youtubeUrl
                });
            }
    ...
    // Save global YouTube settings mutation
    const saveYoutubeSettingsMutation = useMutation({
        mutationFn: async (settings) => {
            if (globalYoutubeSettings?.id) {
                return await base44.entities.YouTubeSettings.update(globalYoutubeSettings.id, settings);
            } else {
                return await base44.entities.YouTubeSettings.create(settings);
            }
        },
        onSuccess: () => {
        ...
        try {
            ...
            if (roomId) {
                updateRoomMutation.mutate({
                    roomId,
                    updates: { channel_avatar_url: file_url }
                });
            } else {
                setNewRoom(prev => ({ ...prev, channel_avatar_url: file_url }));
            }
        } catch (error) {
    ...
    // Group rooms by category and sort by order
    ...
    // Sort rooms within each category by order
    Object.keys(roomsByCategory).forEach(category => {
        roomsByCategory[category].sort((a, b) => (a.order || 0) - (b.order || 0));
    });

    // State tracking for pending mutations per user
    const [pendingMutationIds, setPendingMutationIds] = React.useState({
        blocking: null,
        unblocking: null,
        adminSetup: null,
        userSetup: null
